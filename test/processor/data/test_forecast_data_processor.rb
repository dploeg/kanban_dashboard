...
class TestForecastDataProcessor < Minitest::Test
  ...
  context 'process forecast data' do
    ...
    should 'calculate forecast with different min and max values' do
      ...
      (1..10000).each {
        random_story_value_from_range.expect :call, 30, [@configuration[:forecast_config][:min_number_of_stories], @configuration[:forecast_config][:max_number_of_stories], @forecast_input]
      }
      processor.stub :sample, sample do
        processor.stub :random_story_value_from_range_with_risk, random_story_value_from_range do
          processor.forecast(@forecast_input, @completed_items)
          forecasts = processor.forecasts
    ...
    should 'calculate forecast with risks' do
      @configuration = {:forecast_config => {:start_date => "10/3/16", :min_number_of_stories => 30, :max_number_of_stories => 30,
      ...
      sample                 = MiniTest::Mock.new
      process_risk           = MiniTest::Mock.new
      determine_risk_stories = MiniTest::Mock.new
      #mocking doesn't allow to specify always just do this, so doing it a whole bunch of times
      (1..100000).each {
        sample.expect :call, 4, [@completed_items.values]
      }
      (1..100000).each {
        process_risk.expect :call, true, [{:likelihood => 20, :impact_low => 5, :impact_high => 10}]
        process_risk.expect :call, true, [{:likelihood => 30, :impact_low => 20, :impact_high => 30}]
        process_risk.expect :call, true, [{:likelihood => 75, :impact_low => 10, :impact_high => 15}]
      }
      (1..100000).each {
        determine_risk_stories.expect :call, 5, [{:likelihood => 20, :impact_low => 5, :impact_high => 10}]
        determine_risk_stories.expect :call, 5, [{:likelihood => 30, :impact_low => 20, :impact_high => 30}]
        determine_risk_stories.expect :call, 5, [{:likelihood => 75, :impact_low => 10, :impact_high => 15}]
      }
      processor.stub :sample, sample do
        processor.stub :process_risk?, process_risk do
          processor.stub :determine_risk_stories, determine_risk_stories do
            ...
            (0..100).step(5) do |value|
              forecast = forecasts[("percentile" + value.to_s).to_sym]
              assert_equal Forecast.new(:percentile => value, :duration_weeks => 12, :complete_date => "02/06/16"), forecast
            end
          end
        end
      end
